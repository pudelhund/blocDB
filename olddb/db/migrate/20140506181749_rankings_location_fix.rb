class RankingsLocationFix < ActiveRecord::Migration
  def up
	execute <<-__EOI
...
BEGIN
    ...
    loop   
		...
		left join users u on (bc.user_id = u.id)
		where bpt.status = 0 and bc.event_id = 0 and u.is_visible = true and bpt.location_id = p_row.location_id
		...
		left join boulder_conquerors bc on (bpt.boulder_id = bc.boulder_id) 
		left join users u on (bc.user_id = u.id)
		where bpt.status = 1 and bc.event_id = 0 and u.is_visible = true and bpt.location_id = p_row.location_id
		group by user_id, bc.event_id order by points desc;
